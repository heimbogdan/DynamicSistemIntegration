...
public class WebServiceExtractor {
	...
	private static boolean checkFileAndPath(File file) throws IOException{
		return file.getParentFile().exists() ? true : file.getParentFile().mkdirs() && 
				file.createNewFile();
	}
	
	private static String checkURLFormat(String urlPath){
		return urlPath.endsWith("?wsdl") || 
	...
	public static void extractXSDfromWSDL(File wsdl) throws JAXBException, IOException, TransformerException{
		...
		TDefinitions obj = ((JAXBElement<TDefinitions>) ctx.createUnmarshaller().unmarshal(new FileReader(wsdl))).getValue();
		...
		StringBuilder newPath = new StringBuilder(WS_FOLDER).append(serviceName)
				.append(FW_SLASH).append(serviceName).append(WSDL_EXTENSION);
	...
	private static void getElementsFromTTypes(TTypes types, String serviceName) throws IOException, TransformerException{
		...
		for(Object any : anyList){
			...
			if(elem.getLocalName().equals("schema")){
				if(isLocalDefined(elem)){
					StringBuilder filePath = new StringBuilder(WS_FOLDER).append(serviceName)
							.append(FW_SLASH).append(serviceName).append(UNDER_SCORE)
							.append(index).append(XSD_EXTENSION);
